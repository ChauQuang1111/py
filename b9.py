...
# Định nghĩa cấu trúc ListNode (nút trong danh sách liên kết)
class ListNode:
...
class Solution:
    def swapPairs(self, head: Optional[ListNode]) -> Optional[ListNode]:
        # Tạo node giả để dễ xử lý khi node đầu tiên bị đổi chỗ
        dummy = ListNode(0)
        dummy.next = head

        # 'prev' là node đứng trước cặp đang xử lý
        prev = dummy

        # Lặp đến khi còn ít nhất 2 node để hoán đổi
        while head and head.next:
            first = head          # node thứ nhất trong cặp
            second = head.next    # node thứ hai trong cặp
        ...
        # Trả về node đầu mới sau khi hoán đổi
        return dummy.next
